...
## 2024-03-24 - Missing Firestore Security Rules
**Vulnerability:** The application was writing directly to Firebase Firestore from the client-side SDK (`src/components/ArchiveTerminal.tsx`), but lacked explicit security rules (`firestore.rules` and `firebase.json`). This exposed the `queries` collection to potential unauthorized access, modification, or deletion by anyone who could obtain the Firebase configuration.
**Learning:** Even if the application logic expects users to only access their own data, without explicit server-side rules, the client SDK can be bypassed. Client-side checks are insufficient for data security in direct-to-database architectures like Firebase.
**Prevention:** Always define and deploy strict `firestore.rules` when using the Firebase client SDK for database access. Enforce authentication and authorization (e.g., `request.auth.uid == resource.data.userId`) at the database level, and explicitly deny unintended operations (e.g., `allow update, delete: if false;`).
